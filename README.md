...
- 共有は「submit → 入力」の順: `Share` ボタンを押すと "From Clipboard" / "From File" を選び、内容を受け取り次第即座に送信する(内容が空のまま送信されるバグを構造的に防止)。
- Shareボタンの隣の⚙アイコンから、送信前にカスケードドロップダウンで公開設定・永続化・カスタムURLを設定可能(現在の選択値を常時表示)。
- フォーム全体がドロップターゲット: ファイルをドロップすると「Share!」ボタンのみの確認モーダルが出る(Esc/モーダル外クリックでキャンセル)。
- 一覧の「⋯」メニューから「編集」(内容をモーダルで編集)と「設定」(公開設定/永続化のカスケードドロップダウン)を別々に提供。
- カスタムURL: 共有時に `[A-Z0-9_-]` のIDを指定可能(省略時はランダムID)。
- テーマ: ベース白 + アクセント `#0E172A`。
